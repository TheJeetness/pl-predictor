...
# Club names differ between the FPL feed ("Man City", "Nott'm Forest") and the
# odds feed ("Manchester City", "Nottingham Forest"). Fuzzy word-overlap matching
# is NOT safe here: "Man City"/"Manchester United" share "Manchester", and
# "Coventry City"/"Manchester City" share "City", which silently pairs the wrong
# fixtures. So every spelling maps to one canonical key and matches must be exact.
CANON = {
    "arsenal": "arsenal",
    "aston villa": "villa",
            ...
            for ev in events:
                ...
                for f in fixtures:
                    ...
                    if canon(teams[f["h"]]["name"]) != eh or canon(teams[f["a"]]["name"]) != ea:
                        continue
                    # same two clubs in the same order - confirm the date too, so the
                    # reverse fixture later in the season cannot be picked up by mistake
                    if ek:
                        try:
